...
import { abrahamFaithQuiz } from '@/data/mapLocations';
import { useToast } from '@/hooks/use-toast';

const AbrahamFaithChallenge = () => {
  ...
  const currentQuestion = abrahamFaithQuiz[currentQuestionIndex];
  ...
  const handleNextQuestion = () => {
    if (currentQuestionIndex < abrahamFaithQuiz.length - 1) {
      ...
      toast({
        ...
        description: `You got ${correctAnswers + (selectedOption === currentQuestion.correctAnswer ? 1 : 0)} out of ${abrahamFaithQuiz.length} correct.`,
      });
    }
  };
  ...
  const getScoreMessage = () => {
    ...
    const percentage = (finalScore / abrahamFaithQuiz.length) * 100;
    
    if (percentage >= 90) {
      return "Excellent! You have a deep understanding of Abraham's journey of faith.";
    } else if (percentage >= 70) {
      return "Good job! You have a solid knowledge of Abraham's story and God's promises.";
    } else if (percentage >= 50) {
      return "Nice effort! You're on your way to understanding this important patriarch.";
    } else {
      return "Keep studying! Abraham's story has much to teach us about faith and trusting God's promises.";
    }
  };
  ...
  if (quizCompleted) {
    const finalScore = correctAnswers + (selectedOption === currentQuestion.correctAnswer ? 1 : 0);
    const percentage = Math.round((finalScore / abrahamFaithQuiz.length) * 100);
      ...
      >
        <div className="text-center mb-8">
          ...
          <p className="text-lg text-bible-dark/80">
            You've completed the Faith of Abraham quiz
          </p>
        </div>
        ...
        <div className="glass-card p-8 rounded-xl mb-8">
          <div className="text-center mb-6">
            ...
            <p className="text-xl font-medium">
              You got {finalScore} out of {abrahamFaithQuiz.length} questions correct
            </p>
          </div>
          ...
          <p className="text-center text-bible-dark/70 mb-8">
            Abraham's journey teaches us about faith, obedience, and God's faithfulness to His promises. Through Abraham, God established a covenant that would bless all nations, ultimately fulfilled in Jesus Christ.
          </p>
  ...
  return (
    ...
    >
      <div className="mb-8">
        <h1 className="text-2xl md:text-3xl font-serif font-bold text-bible-dark mb-2">
          Faith of Abraham Quiz
        </h1>
        <p className="text-bible-dark/70">
          Test your knowledge of Abraham's journey and God's promises.
        </p>
        
        <div className="flex items-center justify-between mt-4">
          <div className="text-sm text-bible-dark/60">
            Question {currentQuestionIndex + 1} of {abrahamFaithQuiz.length}
          </div>
          {correctAnswers > 0 && (
        ...
        <div className="w-full bg-gray-200 rounded-full h-2 mt-2">
          <div 
            ...
            style={{ width: `${((currentQuestionIndex) / abrahamFaithQuiz.length) * 100}%` }}
          ></div>
        </div>
      </div>
      ...
      <div className="glass-card p-6 rounded-xl mb-8">
        ...
        {!isAnswerSubmitted ? (
          ...
          >
            {currentQuestionIndex < abrahamFaithQuiz.length - 1 ? (
              <>
                Next Question
...
export default AbrahamFaithChallenge;
